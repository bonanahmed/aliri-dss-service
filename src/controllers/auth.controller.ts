...
const login = catchAsync(async (req: Request, res: Response) => {
  ...
  const tokens = await tokenService.generateAuthTokens(account);
  res.cookie('access_token', tokens.access.token, { maxAge: 86400000, httpOnly: true, domain: '.digibay.id' });
...
const logout = catchAsync(async (req: Request, res: Response) => {
  ...
  res.clearCookie('access_token', { httpOnly: true, domain: '.digibay.id' });
